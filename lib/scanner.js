...
export async function scanTargets(origin, targets, options = {}) {
  const probePath = `/hiddenthings-${randomToken()}-probe`;
  const [baseline, ...results] = await Promise.all([
    checkPath(origin, probePath, options),
    ...targets.map((path) => checkPath(origin, path, options)),
  ]);

  if (!baseline.found) return results;

  return results.map((result) => {
    if (!result.found || result.status !== baseline.status) return result;

    const distinctLength =
      result.contentLength != null &&
      baseline.contentLength != null &&
      result.contentLength !== baseline.contentLength;

    return distinctLength ? result : { ...result, found: false };
  });
}
